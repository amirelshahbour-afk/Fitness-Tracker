...
async function foodPhoto(request,env,user){
 ...
 const reportFields=Object.fromEntries(['age','height','weight','bfr','muscleWeight','bmr'].map(k=>[k,{type:['number','null']}]));
 const reportSchema={type:'object',additionalProperties:false,properties:{is_report:{type:'boolean'},warning:{type:'string'},...reportFields},required:['is_report','warning',...Object.keys(reportFields)]};
 ...
 if(!response.ok){
  ...
  console.warn(JSON.stringify({event:'vision_service_failure',status:response.status,code:typeof code==='string'?code.slice(0,80):'unknown',requestId:response.headers.get('x-request-id')}));
